...
We save first the value of all parameters (those that can be changed) into a variable to be able to reset them to the intial state
...
par(op)
set.seed(1023)
plot(runif(10), runif(10), type = 'n', axes = FALSE, xlim = c(0, 1), ylim = c(0, 1),
    xlab = '', ylab = '')
text(0.5, 0.5, paste0("op$mar = (", paste(op$mar, collapse = ", "), ")"))
box("plot", lty = "44")
for(k in 0:4){
    mtext(paste0("line ", k), line = k, side = 1)
}
for(k in 0:3){
    mtext(paste0("line ", k), line = k, side = 2)
}
for(k in 0:1){
    mtext(paste0("line ", k), line = k, side = 3)
}
for(k in 0:1){
